...
#pragma mark - CalendarView Implementation
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        float yearMonthHeight = frame.size.height * 0.12F;
        float weekHeight = frame.size.height * 0.08F;
        ...
        //0-－30表示最多31天日期字符串
        for(int i = 0; i < 31; i++)
            [_dayAndWeekStringArray addObject: [NSString stringWithFormat:@"%d",i+1]];
        ...
        //TD 把这两个参数去掉，没有必要需要这个参数，或者只是用一个方法来代替这两个参数
        //计算出日期字符串的绘制用尺寸
...
//选中后的字体大小
- (void)newDrawStringWithString:(NSString *)string inRect:(CGRect)rect color:(UIColor *)color
{
    CGSize size = [string sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:18]}];
    CGPoint pos;
    //下面算法是让文字位于要绘制的Rect的水平和垂直中心
    //也就是剧中对齐
    pos.x = (rect.size.width - size.width) * 0.5F;
    ...
    [string drawAtPoint:pos withAttributes:attr];
}
...
-(void) drawYearMonthStr : (NSString*) string rect:(CGRect) rect
{
    ...
    NSDictionary *attr = @{NSFontAttributeName :[UIFont systemFontOfSize:16],
                           NSForegroundColorAttributeName:[UIColor colorWithRed:72/255.0 green:75/255.0 blue:80/255.0 alpha:1]
                           };
    //    CGSize sz = [self getStringDrawingSize:string];
    CGSize sz = [string sizeWithAttributes:attr];
    CGPoint pos;
    pos.y = (rect.size.height - sz.height) * 0.5F;
    float leftSpace = (rect.size.width - sz.width)/2.0;
    pos. x = rect.origin.x + leftSpace;
    CGRect strFrame = CGRectMake(pos.x, pos.y, sz.width, sz.height);
    
    [string drawInRect:strFrame withAttributes:attr];
    
    
    //在日期下面画一条线
    CGRect rectRect = strFrame;
    rectRect.origin.x -= 2;
    rectRect.size.width += 4;
    rectRect.origin.y += 16 + sz.height;
    rectRect.size.height = 1;
    
    UIBezierPath *path = [UIBezierPath bezierPathWithRect:rectRect];
    [[UIColor colorWithRed:255/255.0 green:92/255.0 blue:92/255.0 alpha:1] setFill];
...
-(void) drawRoundRect : (CGRect) rect  radius : (CGFloat)radius
{
    ...
    [[UIColor colorWithRed:255/255.0 green:92/255.0 blue:92/255.0 alpha:0.5] setFill];
    [roundRect fill];
}
...
-(void) drawSelectRange : (selectRange* ) ranges count : (int) count
{
    ...
    if(count == 1)
    {
        [self drawCircleInRect : ranges[0].rect color:[UIColor colorWithRed:255/255.0 green:92/255.0 blue:92/255.0 alpha:1] isFill:YES];
    ...
    for(int i = 0; i < count; i++)
    {
        ...
        if (ranges[i].isStartOrEnd) {
            ...
            [self drawCircleInRect:ranges[i].rect color:[UIColor colorWithRed:255/255.0 green:92/255.0 blue:92/255.0 alpha:1] isFill:YES];
        }
        ...
        else
        {
            ...
            {
                //                [self drawCircleInRect:rc color:[UIColor colorWithRed:255/255.0 green:92/255.0 blue:92/255.0 alpha:1] isFill:YES];
            }
            else
            {
                //一般情况，绘制roundedRect
...
- (void)drawRect:(CGRect)rect
{
    ...
    //        {
    //            //双休日黑色  //!!! 这里修改星期的显示颜色
    //            [self drawStringInRectWithSize:[_dayAndWeekStringArray objectAtIndex:31 + i] rect:rc size:_weekStringDrawingSize color: [UIColor lightGrayColor]];
    //        }
    //        else
    //        {
    //            //其他时间蓝色
    ...
    for(int i = begin ;  i < end; i++)
    {
        ...
        if([self.calendarDelegate isInSelectedDateRange:date])
        {
            ...
            ranges[rangeCount].isStartOrEnd = [self.calendarDelegate isStartOrEndDate:date];
        ...
        else
        {
            ...
            if ([self isBeforeToday:date]) {
                ...
                [self drawStringInRectWithSize:[_dayAndWeekStringArray objectAtIndex: i - _calendar.dayBeginIdx] rect:rc size:_dayStringDrawingSize color:[UIColor colorWithRed:223/255.0 green:223/255.0 blue:223/255.0 alpha:1.0]];
            }
        }
    }
    ...
    for(int i = 0; i < rangeCount; i++)
    {
        ...
        //idx - begin就是当前的要绘制的日期文字的索引号 //选中后字号大一号
        if (!ranges[i].isStartOrEnd) {
            ...
            [self newDrawStringWithString:[_dayAndWeekStringArray objectAtIndex: idx - begin] inRect:ranges[i].rect color:[UIColor whiteColor]];
        }
    }
...
//判断SDate 日期是否  晚于 今天，是的话返回YES，否则返回NO,这个方法是为了将今天之前的日期绘制成不可点击的样式
- (BOOL)isBeforeToday:(SDate)date
{
    NSDate *today = [NSDate date];
    NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    [gregorian setTimeZone: [NSTimeZone systemTimeZone]];
    NSCalendarUnit unitFlags = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay;
    NSDateComponents *todayCom = [gregorian components:unitFlags fromDate:today];
    //今天的年月日
    NSInteger year = [todayCom year];
    NSInteger month = [todayCom month];
    NSInteger day = [todayCom day];
    if (date.year > year) {
        return YES;
